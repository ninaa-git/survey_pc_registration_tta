...
# ------------------------------------------------------------------ config
base_path = (
    "/home/nbodelot/links/projects/def-egranger/nbodelot/registration"
    f"/pc-registration/silico/PARENet/output/P2PSilico/{method}/registration"
)
...
def standard_stats(corruption, severity):
...
def main():
    ...
    for sev in severities:
        ...
        table[sev] = [standard_stats(c, sev) for c in corruptions]

    # ---- write CSV ----
    out_path = os.path.join(os.path.dirname(__file__), f"{metric}_{method}{extenstion}.csv")
