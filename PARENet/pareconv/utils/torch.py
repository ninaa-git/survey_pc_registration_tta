...
def to_cuda(x):
    r"""Move all tensors to cuda asynchronously."""
    if isinstance(x, list):
        return [to_cuda(item) for item in x]
    elif isinstance(x, tuple):
        return tuple(to_cuda(item) for item in x)   # ← also fix the generator bug
    elif isinstance(x, dict):
        return {key: to_cuda(value) for key, value in x.items()}
    elif isinstance(x, torch.Tensor):
        return x.to('cuda', non_blocking=True)       # ← async transfer
    return x


def clean_checkpoint_state_dict(state_dict):
    r"""Normalize checkpoint keys (torch.compile _orig_mod, DDP module., etc.)."""
    model_dict = state_dict['model'] if isinstance(state_dict, dict) and 'model' in state_dict else state_dict
    ...
    for k, v in model_dict.items():
        ...
        while k.startswith('module.'):
            k = k[len('module.'):]
        if k.startswith('rmer.'):
            k = 'transformer.' + k[len('rmer.'):]
        cleaned[k] = v
    return cleaned
...
def load_weights(model, snapshot):
    ...
    model_dict = clean_checkpoint_state_dict(state_dict)
