...
class BaseTrainer(abc.ABC):
    def __init__(
        ...
        if hasattr(self.args, "wandb_name"):
            log_file = osp.join(cfg.log_dir, '{}-{}.log'.format(self.args.wandb_name, time_stamp))
        else:
        ...
        # basic config
        if hasattr(self.args, "corruption"):
            self.snapshot_dir = os.path.join(cfg.ft_snapshot_dir, self.args.corruption, str(self.args.severity))
            print(f"Snapshot directory: {self.snapshot_dir}")
        elif hasattr(self.args, "n_val_samples"):
            self.snapshot_dir = os.path.join(cfg.ft_snapshot_dir, str(self.args.n_val_samples), str(self.args.val_seed))
            self.out_file = os.path.join(self.snapshot_dir, f'output.json')
            print(f"Snapshot directory: {self.snapshot_dir}")
        else:
    ...
    def save_best_snapshot(self, filename):
        if self.local_rank != 0:
            return
        ...
        if self.scheduler is not None:
            state_dict['scheduler'] = self.scheduler.state_dict()
        torch.save(state_dict, filename)
        self.logger.info('Best model saved to "{}"'.format(filename))
    ...
    def load_snapshot(self, snapshot, fix_prefix=True):
        ...
        # in save_snapshot / save_best_snapshot:
        model_state_dict = OrderedDict(
            (self._canon(k), v) for k, v in self.model.state_dict().items()
        )

        # in load_snapshot:
        ckpt = OrderedDict((self._canon(k), v) for k, v in state_dict['model'].items())
        remap = {self._canon(k): k for k in self.model.state_dict().keys()}
        model_dict = OrderedDict((remap.get(k, k), v) for k, v in ckpt.items())
    ...
    def register_optimizer(self, optimizer):
        ...
        if self.distributed and not self.args.resume:
