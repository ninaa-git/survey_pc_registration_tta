...
class EpochBasedTrainer(BaseTrainer):
    def __init__(
        ...
        self.best_metric_rmse = float('inf')
    ...
    def train_epoch(self):
        ...
        for iteration, data_dict in enumerate(self.train_loader):
            ...
            if self.distributed:
                flag = finite.float().detach()
                dist.all_reduce(flag, op=dist.ReduceOp.MIN)   # 0 if ANY rank is non-finite
                all_finite = flag.item() > 0
            else:
                all_finite = bool(finite)

            if not all_finite:
                if not finite:
                    self.logger.warning(f"Non-finite loss at epoch={self.epoch}, iter={self.inner_iteration}, ...")
                self.timer.add_process_time()
                # every rank runs the same fake backward → collective counts stay matched
            ...
            if not torch.isfinite(grad_norm):
                self.logger.warning(
                    f"Non-finite grad norm ({grad_norm}) at epoch={self.epoch}, "
                    f"iter={self.inner_iteration}; skipping optimizer step."
                )
                self.optimizer.zero_grad(set_to_none=True)
                continue
    ...
    def inference_epoch(self):
        ...
        if self.local_rank == 0 and self.writer is not None:
            ...
            rmse = summary_dict.get('RMSE', float('inf'))

            if rmse < self.best_metric_rmse:
                self.best_metric_rmse = rmse
