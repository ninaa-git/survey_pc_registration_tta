...
def extract_correspondences_from_scores(
    ...
    ref_indices = torch.arange(ref_length, device=score_mat.device)
    ...
    if mutual or bilateral:
        ...
        src_indices = torch.arange(src_length, device=score_mat.device)
...
def get_node_correspondences(
    ...
    if ref_knn_masks is None:
        ref_knn_masks = torch.ones(size=(ref_knn_points.shape[0], ref_knn_points.shape[1]), dtype=torch.bool, device=ref_knn_points.device)
    if src_knn_masks is None:
        src_knn_masks = torch.ones(size=(src_knn_points.shape[0], src_knn_points.shape[1]), dtype=torch.bool, device=src_knn_points.device)
...
@torch.no_grad()
def batched_get_node_correspondences(
    ref_nodes,         # [B, M_max, 3] padded
    src_nodes,         # [B, N_max, 3]
    ref_knn_points,    # [B, M_max, K, 3]
    src_knn_points,    # [B, N_max, K, 3]
    transforms,        # [B, 4, 4]
    pos_radius,        # float
    ref_masks,         # [B, M_max]   True for valid superpoints
    src_masks,         # [B, N_max]
    ref_knn_masks,     # [B, M_max, K]
    src_knn_masks,     # [B, N_max, K]
):
    """Returns:
        per_pair_corr_indices:  list of B Long[(C_b, 2)] tensors
        per_pair_corr_overlaps: list of B Float[(C_b,)] tensors
    """
    B = ref_nodes.shape[0]
    device = ref_nodes.device
 
    # Apply per-pair transform to src nodes/points (batched).
    # apply_transform supports (B, N, 3) × (B, 4, 4); reshape (B, M, K, 3) ↔ (B, M*K, 3).
    src_nodes = apply_transform(src_nodes, transforms)                 # [B, N_max, 3]
    B_, N_, K_, _ = src_knn_points.shape
    src_knn_points = apply_transform(
        src_knn_points.reshape(B_, N_ * K_, 3), transforms,
    ).reshape(B_, N_, K_, 3)
 
    # Per-superpoint enclosing-sphere radius
    ref_knn_dists = torch.linalg.norm(ref_knn_points - ref_nodes.unsqueeze(2), dim=-1)  # [B, M, K]
    ref_knn_dists = ref_knn_dists.masked_fill(~ref_knn_masks, 0.0)
    ref_max_dists = ref_knn_dists.max(dim=2)[0]                                          # [B, M]
 
    src_knn_dists = torch.linalg.norm(src_knn_points - src_nodes.unsqueeze(2), dim=-1)
    src_knn_dists = src_knn_dists.masked_fill(~src_knn_masks, 0.0)
    src_max_dists = src_knn_dists.max(dim=2)[0]                                          # [B, N]
 
    # Pairwise distance between superpoints, batched
    dist_mat = torch.sqrt(pairwise_distance(ref_nodes, src_nodes))                       # [B, M, N]
 
    # Enclosing-sphere intersection test
    intersect_mat = (ref_max_dists.unsqueeze(2)
                     + src_max_dists.unsqueeze(1)
                     + pos_radius
                     - dist_mat) > 0                                                      # [B, M, N]
    node_mask_mat = ref_masks.unsqueeze(2) & src_masks.unsqueeze(1)
    intersect_mat = intersect_mat & node_mask_mat
 
    # Single nonzero across the whole batch
    b_idx, ref_idx, src_idx = torch.nonzero(intersect_mat, as_tuple=True)
    if b_idx.numel() == 0:
        empty_corr = torch.zeros((0, 2), dtype=torch.long, device=device)
        empty_overlap = torch.zeros((0,), device=device)
        return [empty_corr] * B, [empty_overlap] * B
 
    # Gather knn data for the selected (b, ref, src) tuples
    sel_ref_knn_points = ref_knn_points[b_idx, ref_idx]   # [V, K, 3]
    sel_src_knn_points = src_knn_points[b_idx, src_idx]
    sel_ref_knn_masks  = ref_knn_masks[b_idx, ref_idx]    # [V, K]
    sel_src_knn_masks  = src_knn_masks[b_idx, src_idx]
 
    point_mask_mat = sel_ref_knn_masks.unsqueeze(2) & sel_src_knn_masks.unsqueeze(1)   # [V, K, K]
 
    # Compute knn-pair distances
    dist_mat = pairwise_distance(sel_ref_knn_points, sel_src_knn_points)                # [V, K, K]
    dist_mat = dist_mat.masked_fill(~point_mask_mat, 1e12)
    point_overlap_mat = dist_mat < pos_radius ** 2                                       # [V, K, K]
    ...
    ref_overlaps = ref_overlap_counts / sel_ref_knn_masks.sum(-1).clamp(min=1).float()
    src_overlaps = src_overlap_counts / sel_src_knn_masks.sum(-1).clamp(min=1).float()
    overlaps = (ref_overlaps + src_overlaps) / 2                                         # [V]
 
    keep = overlaps > 0
    b_idx_keep   = b_idx[keep]
    ref_idx_keep = ref_idx[keep]
    src_idx_keep = src_idx[keep]
    overlaps_keep = overlaps[keep]
 
    # Split per pair using bucket sort by b_idx
    per_pair_corr_indices = [None] * B
    per_pair_corr_overlaps = [None] * B
    for b in range(B):
        sel = b_idx_keep == b
        ref_b = ref_idx_keep[sel]
        src_b = src_idx_keep[sel]
        per_pair_corr_indices[b] = torch.stack([ref_b, src_b], dim=1)
        per_pair_corr_overlaps[b] = overlaps_keep[sel]
 
    return per_pair_corr_indices, per_pair_corr_overlaps
