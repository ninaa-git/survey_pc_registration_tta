...
from pareconv.modules.transformer.rpe_transformer_LN import AdaptiveLayerNorm, AdaptiveAttentionOutput
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None):
        ...
        self.scale = float(self.d_model_per_head) ** -0.5
...
class AttentionLayer_LN(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, ctx=None):   # add ctx=None
        super(AttentionLayer_LN, self).__init__()
        self.attention = MultiHeadAttention(d_model, num_heads, dropout=dropout)
        self.linear    = nn.Linear(d_model, d_model)
        self.dropout   = build_dropout_layer(dropout)
        if ctx is not None:
...
class TransformerLayer_LN(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU', ctx=None):  # add ctx=None
        super(TransformerLayer_LN, self).__init__()
        self.attention = AttentionLayer_LN(d_model, num_heads, dropout=dropout, ctx=ctx)   # pass ctx
        # output sublayer always uses plain AttentionOutput — AdaptiveLayerNorm is NOT
        # used here.  output.norm receives features already processed by attention.norm.
        # If attention.norm corrects the domain shift, output.norm's input is no longer
        # the raw corrupted distribution, so stats collected from a non-adapted forward
        # would be wrong.  Correction at attention.norm is sufficient.
