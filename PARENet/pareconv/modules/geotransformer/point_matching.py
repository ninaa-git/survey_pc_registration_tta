...
class PointMatching(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        ref_indices = torch.arange(ref_length, device=device).view(1, ref_length, 1).expand(batch_size, -1, self.k)  # (B, N, K)
        ...
        src_indices = torch.arange(src_length, device=device).view(1, 1, src_length).expand(batch_size, self.k, -1)  # (B, K, N)
