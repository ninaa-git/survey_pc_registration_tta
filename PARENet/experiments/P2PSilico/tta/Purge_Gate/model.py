...
class PARE_Net(nn.Module):
    ...
    def forward_prototype_purge(self, data_dict, feats_key, source_stats, purge_size):
        ...
        m = valid.unsqueeze(-1).to(features.dtype)              # [B, N, 1]
        counts = m.sum(dim=1, keepdim=True).clamp_min(1.0)      # [B, 1, 1]
        feat_mean = (features * m).sum(dim=1, keepdim=True) / counts          # [B, 1, C]
        feat_var  = (((features - feat_mean) * m) ** 2).sum(dim=1, keepdim=True) / counts
        feat_std  = (feat_var.clamp_min(0.0) + 1e-12).sqrt()                  # [B, 1, C]

        diff            = features - source_mean                              # [B, N, C]
        ...
        real_per_b = corr_real_mask.sum(dim=1)
