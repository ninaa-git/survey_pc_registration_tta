...
def run(args, cfg):
    ...
    clean_loader, cfg.backbone.num_neighbors = corrupted_test_data_loader(clean_args, cfg)
    cor_loader,   _                          = corrupted_test_data_loader(args, cfg)

    # early_stop_module: abort forward right after model.transformer finishes
    # (AdaptiveLayerNorm hooks inside it have already fired at that point)
    stop = model.transformer

    F_clean = core.collect_with_hook(root, clean_loader, sel, to_cuda, n_clouds,
                                     sample_idx=sample_idx,
                                     forward_fn=fwd, early_stop_module=stop,
                                     ctx=ctx, stream=None)
    clear()
    ...
    # ── No-adapt RMSE — computed HERE, before any LN_TTA_Manager is registered,
    #    so the full forward runs with plain LN (no adaptive stats injected).
    ...
    # ── Adapted RMSE — manager is still active (stats warmed up from F_adapt
    #    pass), 1 pair gives a concrete per-sample value matching the plot.
