...
class PARE_Net(nn.Module):
    ...
    def _select_corrs(self, B, gt_corr_per_pair, gt_overlap_per_pair,
        ...
        if self.training:
            # Single sync: per-pair counts of valid predicted corrs, all at once.
            valid_counts = corr_real_mask.sum(dim=1).tolist()   # one D2H copy for all B
            for b in range(B):
