...
class Tester(SingleTester):
    def __init__(self, cfg):
        ...
        if self.args.align_weight > 0.0:
            stats_file = osp.join(self.args.align_stats_dir, 'source_feats.pth')
            ...
            if not osp.exists(stats_file):
                ...
                try:
                    ...
                    generate_pea_stats(self.args, cfg, model=self.model)
        ...
        if cfg.data.dataset == 'real':
            out_file = osp.join(self.output_dir, f'test_{cfg.data.dataset}_{self.args.align_momentum}.json')
        else:
            out_file = osp.join(self.output_dir, f'test_{self.args.corruption}_{self.args.severity}_{self.args.align_momentum}.json')
