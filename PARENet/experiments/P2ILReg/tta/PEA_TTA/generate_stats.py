...
def _collect_stats_online(model, cfg, loader, stage_keys, n_batches=None):
    ...
    from tqdm import tqdm
    pbar = tqdm(loader, total=total, desc="collecting",
    ...
    for batch_idx, data_dict in enumerate(pbar):
        ...
        pbar.set_postfix(pts=acc[stage_keys[0]]['n'])
...
# ─────────────────────────────────────────────────────────────────────────────
def runner(args, config, model=None):
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    if model is None:
        # Standalone invocation only. This raw strict=False load does NOT apply
        # the tester's key cleaning — prefer passing the tester's loaded model.
        ...
        logger.info("Using the tester's pre-loaded model (no reload).")
    model.eval()

    # ── Source loader ────────────────────────────────────────────────────────
    logger.info("Loading source data … (building dataset index, this can take a while)")
    t0 = time.time()
    syn_or_real = config.data.dataset
    config.data.dataset = 'syn'
    src_loader, _, cfg_neighbors = train_valid_data_loader(config, False)
    config.data.dataset = syn_or_real
    config.backbone.num_neighbors = cfg_neighbors
    logger.info(f"Dataset ready in {time.time()-t0:.1f}s — {len(src_loader)} batches")

    # ── Collect SOURCE statistics only ───────────────────────────────────────
    n_batches = getattr(args, 'n_batches', None)   # None = full dataset
    force     = getattr(args, 'force', False)

    src_cache = f"intermediate_features/pea_stats/source_feats.pth"
