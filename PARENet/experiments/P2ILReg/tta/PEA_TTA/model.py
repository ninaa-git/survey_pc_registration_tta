...
class PARE_Net(nn.Module):
    ...
    def set_alignment_stats(self, src_stats: dict,
        ...
        def _to_cuda_float(x):
            if not isinstance(x, torch.Tensor):
                x = torch.from_numpy(x)
            return x.float().cuda()

        self._align_stats = {
            'src_mean':        _to_cuda_float(src_stats['mean']),
            'src_cov_sqrt':    _to_cuda_float(src_stats['cov_sqrt']),
            'tgt_mean':        None,   # estimated online via EMA
            'tgt_cov':         None,
        ...
        print(f'src_mean: {self._align_stats["src_mean"]}, tgt_mean: None (EMA)')
    ...
    def _wct_align(feats: torch.Tensor,
        ...
        """
        Whitening-Coloring Transform for a single feature matrix (PEA eq. 3-4).

            Y = (F - μ_t) Σ_t^{-1/2} Σ_s^{1/2} + μ_s          (eq. 3)
            F' = (1 - w) F + w Y                                  (eq. 4)
        ...
        """
        # Whiten: remove target-domain mean and decorrelate
        whitened = (feats - tgt_mean.unsqueeze(0)) @ tgt_cov_invsqrt   # [N, C]
        # Re-color: impose source-domain covariance and mean
    ...
    def forward(self, data_dict):
        ...
        # PEA alignement of coarse features
        ...
        ref_lens_cpu   = data_dict['ref_lens'].tolist()
        ...
        ref_off   = data_dict['ref_off'].tolist()
        ...
        # PEA alignment of coarse features
        if self._align_stats is not None and not self.training:
            s = self._align_stats

            # ── Compute batch statistics from valid ref features ──────────
            B_sz, ref_N, C = ref_feats_c_pad.shape
            mask_f = ref_mask_c.float().unsqueeze(-1)      # [B, ref_N, 1]
            n_valid = mask_f.sum()

            flat   = (ref_feats_c_pad * mask_f).reshape(-1, C)
            ...
            ridge = 1e-3 * cov.diagonal().mean().clamp_min(1e-8)        # conditioning
            cov   = cov + ridge * torch.eye(C, device=cov.device, dtype=cov.dtype)

            try:
                cov64 = cov.double()                          # fp64 on GPU (no .cpu() sync)
                evals, evecs = torch.linalg.eigh(cov64)
                evals = evals.clamp(min=1e-6)
                tgt_cov_invsqrt = ((evecs * (1.0 / evals.sqrt()).unsqueeze(0)) @ evecs.T
                                   ).to(dtype=cov.dtype)
            except torch.linalg.LinAlgError:
                tgt_cov_invsqrt = torch.eye(C, device=cov.device, dtype=cov.dtype)  # skip alignment this step

            s['tgt_cov_invsqrt'] = tgt_cov_invsqrt

            # ── Apply WCT alignment to both ref and src padded features ──
            tgt_mean = s['tgt_mean']                             # [C]

            B_sz, ref_N, C = ref_feats_c_pad.shape
            ref_feats_c_pad = self._wct_align(
                ref_feats_c_pad.reshape(-1, C),
                src_mean=s['src_mean'],
        ...
        batched = {
            ...
            'src_points_per_pair':      [points[src_off[b]:src_off[b] + src_lens_cpu[b]]
                                         for b in range(B)],
            'ref_points_per_pair':      [points[ref_off[b]:ref_off[b] + ref_lens_cpu[b]]
                                         for b in range(B)],
        }

        return {'batch_size': B, 'batched': batched}

    def forward_out_intermediate(self, data_dict):
        re_feats_f, feats_f, _, feats_c, _ = self.backbone(data_dict)
        B = data_dict['batch_size']
        device = feats_c.device
        ref_lens_c = data_dict['ref_lens_c'].to(device)
        ref_lens_f = data_dict['ref_lens_f'].to(device)
        ref_off_c  = data_dict['ref_off_c'].to(device).long()
        ref_off_f  = data_dict['ref_off_f'].to(device).long()
        # Build flat index lists selecting only the ref rows out of the stacked tensors
        ref_idx_c = torch.cat([
            torch.arange(ref_off_c[b], ref_off_c[b] + ref_lens_c[b], device=device)
            for b in range(B)
        ])
        ref_idx_f = torch.cat([
            torch.arange(ref_off_f[b], ref_off_f[b] + ref_lens_f[b], device=device)
            for b in range(B)
        ])
        return {
            'ref_feats_c': feats_c[ref_idx_c],   # ref-only
            'ref_feats_f': feats_f[ref_idx_f],
        }
