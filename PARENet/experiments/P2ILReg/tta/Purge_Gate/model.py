...
class PARE_Net(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if P_max == 0:
            raise RuntimeError(
                f"No correspondences produced for this batch (B={B}). "
                f"P_per_pair={P_per_pair_list}. Point cloud has no usable output."
            )
    ...
    def forward_out_intermediate(self, data_dict):
        ...
        output_dict['b_ref_feats_c_pad']= ref_feats_c_pad
        output_dict['b_ref_masks_c']= ref_mask_c
        output_dict['batch_size']= B

        return output_dict


    def forward_prototype_purge(self, data_dict, feats_key, source_stats, purge_size):
        B = data_dict['batch_size']
        device = data_dict['features'].device
        ...
        # --- PURGE SECTION ---
        features = ref_feats_c_pad                              # [B, N_c_max, C]
        B, N, C = features.shape
        device = features.device

        valid = ref_mask_c                                      # [B, N_c_max]  bool

        source_mean = source_stats['mean'].to(features.dtype).view(1, 1, -1)
        source_std  = source_stats['std'].to(features.dtype).view(1, 1, -1)

        fm = features.mean(dim=-1, keepdim=True)                # per-token, across channels
        fs = features.std(dim=-1,  keepdim=True)
        features_norm = (features - fm) / (fs + 1e-6) 

        diff            = features_norm - source_mean                              # [B, N, C]
        normalized_diff = diff / (source_std + 1e-8)
        divergence      = torch.linalg.norm(normalized_diff, dim=-1)          # [B, N]

        divergence = divergence.masked_fill(~valid, float('-inf'))

        n_valid_per_b = valid.sum(dim=1)                                      # [B]
        n_purge_per_b = (n_valid_per_b.float() * purge_size).long()           # [B]

        _, sorted_idx = divergence.sort(dim=1, descending=True)               # [B, N]
        arange_n = torch.arange(N, device=device).unsqueeze(0)                # [1, N]
        top_in_sorted = arange_n < n_purge_per_b.unsqueeze(1)                 # [B, N]

        purge_mask = torch.zeros_like(valid)
        purge_mask.scatter_(1, sorted_idx, top_in_sorted)

        ref_mask_c = valid & ~purge_mask
        # --- END COARSE PURGE ---

        # 3. Conditional Transformer # mask already implemented in the conditional transformer      
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            ref_feats_c_t, src_feats_c_t, _ = self.transformer(
                ref_points_c_pad, src_points_c_pad,
                ref_feats_c_pad,  src_feats_c_pad,
        ...
        batched = {
            ...
            'ref_points_per_pair':      [points[ref_off[b]:ref_off[b] + ref_lens_cpu[b]]
                                         for b in range(B)],
            'ir_final_per_pair':        [h[-1] for h in hyp_outputs],
        }
