"""
Compute and cache embedding statistics or raw embeddings for PAREConv features.

Usage:
    # Mean/std stats for purge (single or multiple features)
    python embedding_stats.py --checkpoint path/to/model.pth --features ref_feats_c src_feats_c

    # One mean/std per point cloud (e.g. for nearest-neighbour purge per cloud)
    python embedding_stats.py --checkpoint path/to/model.pth --features ref_feats_c --mode stats_per_cloud

    # One aggregated vector per point cloud (e.g. for cluster visualisation)
    python embedding_stats.py --checkpoint path/to/model.pth --features ref_feats_c --mode per_cloud

    # Every point with its point-cloud label (e.g. for point-level visualisation)
    python embedding_stats.py --checkpoint path/to/model.pth --features ref_feats_c --mode per_point
"""
...
if __name__ == "__main__":
    ...
    if args.output is None:
        args.output  = f"intermediate_features/purge_gate_stats/source_feats_{cfg.data.dataset}.pth"
