...
_C.exp_name    = os.environ.get("TTA_METHOD", osp.basename(_C.working_dir))
_C.dataset_name = os.environ.get("TTA_DATASET", "P2ILReg")
_C.output_dir       = osp.join(_C.root_dir, 'output', _C.dataset_name, _C.exp_name)
_C.snapshot_dir     = osp.join(_C.output_dir, 'snapshots')
_C.ft_snapshot_dir     = osp.join(_C.output_dir, 'ft_snapshots')
...
_C.ft_validation_dir = osp.join(_C.output_dir, 'ft_validation')
_C.viz_dir = osp.join(_C.output_dir, 'viz')
...
ensure_dir(_C.viz_dir)
# data
_C.data = edict()
_C.data.dataset_root ='../../../data/Liver_regis' #str(Path(os.environ["SCRATCH"]) / "data" / "Liver_regis") #'../../../data/Liver_regis' #str(Path(os.environ["SCRATCH"]) / "data" / "Liver_regis") #
_C.data.cor_dataset_root = '../../../data/corruptions/data/datasets_corruptions_p2ilreg' # #'../../../data/corruptions/datasets_corruptions_p2ilreg'
_C.data.voxel_size = 0.02
_C.data.dataset = 'real' #real or syn

# train data
_C.train = edict()
_C.train.batch_size =1 #6
_C.train.num_workers =4 #10
_C.train.point_limit = 8192


# test data
_C.test = edict()
_C.test.batch_size =1 #6
_C.test.num_workers =4 #10
_C.test.point_limit = 8192 
...
_C.optim = edict()
_C.optim.lr = 7e-4
_C.optim.lr_decay = 0.97
_C.optim.lr_decay_steps = 3
...
_C.backbone.num_neighbors = [35] * _C.backbone.num_stages  # we used constant neighbors
...
_C.backbone.init_radius = _C.backbone.init_voxel_size * 2.5
...
_C.geotransformer.sigma_d = 0.08
...
_C.fine_matching.acceptance_radius = 0.04
...
_C.coarse_loss.positive_overlap = 0.05

# loss - Fine level
_C.fine_loss = edict()
_C.fine_loss.positive_radius = 0.03
_C.fine_loss.negative_radius = 0.08
